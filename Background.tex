...
Voxelwise FWE control is established using an intrinsic relationship linking the probability of making a false discovery with the distribution of the maximum $t$-statistic over the brain. Defining the global null hypothesis of no signal anywhere, $H_{0,G}: (\forall \bm{s} \in S :\bm{c}^{\intercal}\bm{\beta}(\bm{s}) = 0)$, then for a threshold $c$, 

\begin{equation}
\label{eq:FWE_and_global_maximum}
P(\text{Reject} \ H_{0,G} | H_{0,G} \ \text{true}) = P\Big(\max\limits_{\bm{s} \in S} T(\bm{s}) > c \Big)
\end{equation}
where $T(\bm{s})$ is the $t$-statistic map given in \ref{eq:t_statistic}. 

The intuition of RFT is that under the global null, $T(\bm{s})$ can be modelled by a stationary continuous Gaussian random field $Z(\bm{s})$ of mean zero and unit variance over the same domain $S$. Note that this imposes the assumptions that the data are stationary and that the discretely sampled statistic image $T(\bm{s})$ is sufficiently smooth enough to be approximated by a continuous field. It is because of the latter that spatial smoothing of the data is essential during preprocessing.  Supposing that the model is valid, then the right-hand side of \ref{eq:FWE_and_global_maximum} is approximated by $P\Big(\max\limits_{\bm{s} \in S} Z(\bm{s}) > c \Big)$. Remarkably, this probability can be obtained with use of the \textit{Euler characteristic} (EC), a measure originating from algebraic topology to provide information about a shape's structure that is invariant under any continuous deformation applied to the shape. 

For a threshold $c$, defining the \textit{excursion set} $A_{c}$ as the set of voxels where the random field exceeds $c$, i.e. $A_{c} = \Big\{ \bm{s} \in S : Z(\bm{s}) > c \Big\}$, the EC $\chi_{c}$ is characterized as counting the number of clusters, minus the number of `handles', plus the number of `holes' in $A_c$. If $c$ is chosen large enough then we expect the handles and holes to disappear, so the EC provides an approximation of the number of clusters. If $\max\limits_{\bm{s} \in S} Z(\bm{s}) > c$ then clearly there must be at least one suprathreshold cluster in the excursion. Putting all this information together,
...
Finally, further increasing of the threshold $c$ will result in fewer voxels contained in the excursion set until $\chi_{c}$ will almost certainly take the value of 1 (if the excursion set is made up of one suprathreshold cluster) or 0 (if the excursion set is empty). 
